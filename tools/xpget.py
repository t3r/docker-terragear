#!/usr/bin/env python
#based on https://github.com/accek/fg-nav-fixups/blob/master/xplane-apts/xplane-get-apts
...
def load_json(url_suffix, filename_suffix):
	...
	try:
		return json.load(open(filename)),True
	except Exception:
...
meta,cached = load_json('/apiv1/airports', 'meta.json')
...
for airport in meta['airports']:
	...
	try:
		...
		scenery_path = 'cache/%d-%s.json' % (scenery_id, code)
		scenery,cached = load_json('/apiv1/scenery/%d' % (scenery_id,), scenery_path)
		print >>sys.stderr, code, "Cached" if cached else "New"
		...
		if False == cached:
			aptdatfilename = os.path.join(args.output_dir,'dat/%s.dat') % (code,)
			try:
				os.makedirs(os.path.dirname(aptdatfilename))
			except OSError:
				pass
			aptdatfile = open(aptdatfilename, 'w')
			aptdatfile.write("\n".join(dat[0:-1]) + '\n')
			aptdatfile.write('99\n')
			aptdatfile.close()
	except Exception:
